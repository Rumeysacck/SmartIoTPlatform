  ...
  return (
    <div style={{ padding: '30px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <h1 style={{ color: '#1a1a1a', margin: 0 }}>Ana Kontrol Paneli</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px' }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            <DeviceActionRow name="Aydınlatma Modülü" online={connectionStatus.lightModule} onConnect={() => handleReconnect('lightModule')} />
            <DeviceActionRow name="Sıcaklık/Nem Sensörü" online={connectionStatus.environmentSensors} onConnect={() => handleReconnect('environmentSensors')} />
            <DeviceActionRow name="Giriş Kontrol Sistemi" online={connectionStatus.entrySystem} onConnect={() => handleReconnect('entrySystem')} />
          </div>
        </div>
...
const DeviceActionRow = ({ name, online, onConnect }) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', borderRadius: '10px', backgroundColor: '#f8fafc', border: `1px solid ${online ? '#e2e8f0' : '#fecaca'}` }}>
        ...
        {!online ? (
            <button onClick={onConnect} style={smallBtnStyle}><RefreshCw size={12} /> Bağlan</button>
        ) : (
...
const btnStyle = { padding: '10px 25px', borderRadius: '20px', border: 'none', color: 'white', cursor: 'pointer', fontWeight: 'bold' };
